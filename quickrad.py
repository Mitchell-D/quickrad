...
from sbdart import dispatch_sbdart, parse_iout
iout_id = 7
output = dispatch_sbdart({"iout":iout_id}, Path("/tmp"))
data = parse_iout(iout_id=iout_id, output)
```
After running this code, 'data' will contain a dictionary
of data for output style 7 and all-default settings.

Parsing methods have been written for most of the output modes,
including a few for multiprocessed lookup table generation.
"""
from pathlib import Path
...
def sflux_over_fields(fields, new_coords, sbdart_args, tmp_dir_parent,
    ...
    """
    Run SBDART to solve for spectral flux in (W/m^2/um) over a user
    specified coordinate space. This method uses multiprocessing.

    :@param fields: String SBDART parameter fields in an order corresponding
        to the "new_coords" parameter arguments.
    :@param new_coords: List of lists containing discrete parameter values at
        which to SBDART will be evaluated. The order of the outside list
        corresponds to axis order of each field in the returned array, and the
        order of the inside lists determines the index order for the axis.
    :@param sbdart_args: Dictionary of any static SBDART parameters not
        addressed as a coordinate.
    :@param workers: Maximum number of subprocesses to spawn
    :@param dtype: data type for lookup table

    :@return: 3-tuple (labels, coords, sflux)
        labels := List of string labels naming each axis
        coords := Nested lists of coordinate values or labels for each axis
        sflux  := A (c1, ..., cN, wl, z, feats) shaped array for each of the N
                  user-specified coordinate axes and the (wl, z, feats)
                  coordinates from arrays returned with SBDART iout=7
    """
    new_coords = list(map(list, new_coords))
    assert len(fields)==len(new_coords)
    ...
    arg_space = np.full([len(new_coords[i]) for i in range(len(fields))], True)
    coord_idxs = list(zip(*np.where(arg_space)))
    # Make a SBDART argument dictionary with each combination
    fields_coords = [
            {fields[i]:new_coords[i][aidx[i]] for i in range(len(aidx))}
            for aidx in coord_idxs
            ]
    # Choose a temporary directory hashstring
    ...
    full_args = (({**sbdart_args, "iout":7, **fields_coords[i]},
                  tmp_dirs[i])
            for i in range(len(fields_coords)))

    ## Set variable to store coords for each axis of arays subprocesses return
    sub_coords = None
    sflux = None
    try:
        ## Multiprocess the model run at each coordinate combination point
        with Pool(workers) as pool:
            for sb_in,sb_out in pool.imap(_mp_sbdart, full_args):
                ## _mp_sbdart returns input and output of model as a 2-tuple
                sb_args,sb_path = sb_in
                ## Use the arguments to determine the Nd index of
                ## this new-coordinate point
                new_coord_idx = tuple(
                        new_coords[fields.index(f)].index(sb_args[f])
                        for f in fields
                        )
                ## At the first pass, capture the invariant coordinate values
                ## for each axis of the array returned by each process
                if sub_coords is None:
                    ...
                    ## and the (wl, z, feats) coordinates from SBDART iout=7
                    coords = list(new_coords) + sub_coords
                    sflux_shape = [len(c) for c in coords]
                    sflux = np.full(sflux_shape, np.nan, dtype=dtype)

                ## Move the feature axis to last so array is like (wl,z,feats)
                sub_arr = np.transpose(np.stack(sb_out["sflux"]), axes=(1,2,0))
                ## Load this model run's sub-array into the full table
                sflux[*new_coord_idx] = sub_arr
    except Exception as E:
        raise E
    labels = list(fields) + ["wl", "z", "feats"]
    return labels, coords, sflux

def flux_over_fields(fields, coords, sbdart_args, tmp_dir_parent, workers=5):
    """
    Wrapper method for evaluating for wavelength-integrated radiative flux
    with SBDART setting iout=10.
    """
...
def _parse_iout7(sb_out:list):
    ...
    ## Check for SBDART runtime errors and propagate them
    if records[0][0] in ("CHKIN","Input"):
        print(records[1])
        raise ValueError(" ".join(" ".join(map(str,r)) for r in records))
    nz = int(records[0][1])
...
def parse_iout(iout_id:int, sb_out:list, print_stdout:bool=False):
    """
